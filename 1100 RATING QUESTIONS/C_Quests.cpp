...
void solve()
{
    ...
    for (int i = 0; i < n && ops > 0; i++)
    {
        ...
        ll remaining_ops = ops;
        ll container = prefs + max_b * remaining_ops;

        mcontainer = max(mcontainer, container);
    }

    if (k > n)
    {
        mcontainer = max(mcontainer, prefs + (k - n) * max_b);
    }
